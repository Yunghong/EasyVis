...
std::vector<Object> YoloV8::detectObjects(const cv::cuda::GpuMat &inputImageBGR) {
    ...
    if (numOutputs == 1) {
        ...
        // by pass detecting whether pose or object detection
        // if (numChannels == 18) {
        //     // Pose estimation
        //     ret = postprocessPose(featureVector);
        // } else {
...
std::vector<Object> YoloV8::postprocessPose(std::vector<float> &featureVector) {
    ...
    for (int i = 0; i < numAnchors; i++) {
        ...
        auto bboxesPtr = rowPtr;
        auto scoresPtr = rowPtr + 4;
        auto kps_ptr = rowPtr + 4 + numClasses;
...
// handle outputs of YOLO, summarize them using our containers
void YoloV8::summaryResults(
    ...
    for (auto & object : objects) {
        // std::cout<<"object.label "<<object.label<<std::endl;
        // Pose estimation
        if (!object.kps.empty()) {
            auto& kps = object.kps;
            auto& bbx = object.rect;

            // bean
            if(object.label==0){
                bean2d beanTmp;

                beanTmp.beanCenter=cv::Point2i(std::round(kps[0]),std::round(kps[1]));
                beanTmp.beanCenterValid=(kps[2] > KPS_THRESHOLD);
                beanTmp.bbox=bbx;

                Bean_Black.push_back(beanTmp);
            }
            ...
            if(object.label==1){
                ...
                Bean_DarkBlue.push_back(beanTmp);
            }
            ...
            if(object.label==4){
                ...
                Bean_LightBlue.push_back(beanTmp);
            }
            ...
            if(object.label==7){
                ...
                Bean_Red.push_back(beanTmp);
            }

            // grasper
            if(object.label==8){
                grasper2d grasperTmp;
                
                // locator
                grasperTmp.locator=cv::Point2i(std::round(kps[0]),std::round(kps[1]));
                grasperTmp.locatorValid=(kps[2] > KPS_THRESHOLD);

                if(!grasperTmp.locatorValid || grasperTmp.locator.x<5 || grasperTmp.locator.x>635 || grasperTmp.locator.y<5 || grasperTmp.locator.y>475){
                    continue;
                }

                // joint
                grasperTmp.joint=cv::Point2i(std::round(kps[3]),std::round(kps[4]));
                grasperTmp.jointValid=(kps[5] > KPS_THRESHOLD);

                // left finger
                grasperTmp.leftFinger=cv::Point2i(std::round(kps[6]),std::round(kps[7]));
                grasperTmp.leftFingerValid=(kps[8] > KPS_THRESHOLD);

                // right finger
                grasperTmp.rightFinger=cv::Point2i(std::round(kps[9]),std::round(kps[10]));
                grasperTmp.rightFingerValid=(kps[11] > KPS_THRESHOLD);

                grasperTmp.bbox=bbx;
                
                Grasper_B.push_back(grasperTmp);
            }
            ...
            if(object.label==9){
                ...
                Grasper_W.push_back(grasperTmp);
            }
        }
    }
}
...
void YoloV8::drawObjectLabels(cv::Mat& image, const std::vector<Object> &objects, unsigned int scale) {
    ...
    for (auto & object : objects) {
        ...
        cv::putText(image, text, cv::Point(x, y + labelSize.height), cv::FONT_HERSHEY_SIMPLEX, 0.5 * scale, txtColor, scale);
